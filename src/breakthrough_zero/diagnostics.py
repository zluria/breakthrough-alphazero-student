...
def generate_solver_examples(
    ...
    *,
    ...
    while min(len(buckets[1]), len(buckets[-1])) < target_per_label:
        ...
        buckets[label].append((canonical_planes(game), policy, relative_value))
...
def evaluate_tactical_suite(
    ...
    *,
    ...
    solver = AlphaBetaAgent(depth=solver_depth)
    rows: list[dict] = []
    for item in tactical_suite():
        expected_value, expected_move = solver.search(item.game.clone(), solver_depth)
        ...
        rows.append(
            {
                ...
                "solver_action": item.game.encode_move(expected_move),
                "top_policy_action": predicted_action,
                "policy_correct": predicted_action == item.game.encode_move(expected_move),
            }
        )
    paired_consistency = []
...
def run_supervised_diagnostic(
    ...
    *,
    ...
    report = {
        ...
        "passed": tactics["value_accuracy"] >= 0.7
        and tactics["policy_accuracy"] >= 0.4
