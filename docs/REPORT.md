...
The final numerical summary is inserted only from completed JSON and Slurm artifacts. The rules, search, replay, tactical, perspective, and network save/load invariants are covered by the test suite. Before the current architecture cleanup, the revised solver-supervised diagnostic reached 90.2% held-out value accuracy and 60.0% held-out policy accuracy. The assignment's dummy-PUCT stage completed 10,000 games and 121,565 reconstructable positions. Neural pretraining on a whole-game split finished with validation policy/value losses of 2.233/0.746. Twenty self-play iterations then produced the accepted 5x5 checkpoint. In formal paired-opening arenas it scored 100% against random, 66% against alpha-beta, 86% against rollout PUCT, 65% against the pretrained network, and 63% against the first learned checkpoint.
...
The final 5x5 run, Slurm job 33986, completed 20 iterations, 1,280 games, and 16,704 fresh positions. Its accepted iteration-19 checkpoint had tactical mean signed value 0.8841, 95% sign accuracy, 100% policy accuracy, and zero color-swap error. In Slurm job 33994, six 100-game comparisons used 50 unseeded four-ply openings with colors exchanged and 0.1 seconds per move. The accepted checkpoint scored 100% against random, 66% against alpha-beta, 86% against rollout PUCT, 65% against pretraining, and 63% against iteration 0; all five 95% Wilson intervals excluded 50%. It scored 46% against a later continuation checkpoint, with interval 36.6%-55.7%, so that comparison is inconclusive. The continuation was rejected by the fixed tactical-retention condition, not by its arena score.

Native 8x8 smoke job 33995 completed the full pipeline boundary successfully. Full job 33996 is generating a new native 8x8 corpus and will train a new native 8x8 model from scratch; no 5x5 data or weights are transferred. No 8x8 playing-strength result is reported while that job is running.
